...
interface Applicant {
  ...
  appliedDate: string;
  status: ScreeningStatus;
  // Screening score data (shown in the detail drawer)
  creditScore: number;       // e.g. 720
  creditVerified: boolean;
  idVerified: boolean;
  incomeRatio: number;       // e.g. 0.28 means 28% of income goes to rent
  monthlyIncome: number;
  monthlyRent: number;
  notes: string;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Mock data — in production these come from GET /api/tenants/applications
// ─────────────────────────────────────────────────────────────────────────────
...
interface DrawerProps {
  applicant: Applicant;
  onClose: () => void;
  /** Navigates the owner dashboard to the e-sign section for this applicant */
  onProceedToESign: (applicant: Applicant) => void;
  /** Updates the applicant status to 'Action Required' (declined) */
  onDecline: (id: string) => void;
}

const ApplicantDrawer: React.FC<DrawerProps> = ({ applicant: a, onClose, onProceedToESign, onDecline }) => {
  const ratioGood = a.incomeRatio <= 0.33;
  ...
  return (
    <>
      ...
      {/* Slide-in panel */}
      <aside className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 flex flex-col animate-slideInRight overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-6 py-5 space-y-4 border-b border-zinc-100">
            ...
            <ScoreMeter
              label={`Income-to-Rent Ratio (${Math.round(a.incomeRatio * 100)}%)`}
              value={Math.round(a.incomeRatio * 100)}
              max={50}
              color={ratioGood ? 'bg-emerald-500' : 'bg-amber-400'}
            />
            ...
            <div className="space-y-2 pt-1">
              {[
                ...
                <div key={c.label} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border ${
                  c.pass ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'
                }`}>
                  {c.pass
                    ? <CheckCircle2 className="h-4 w-4 text-emerald-600 shrink-0" />
                    : <AlertCircle  className="h-4 w-4 text-rose-500 shrink-0" />}
                  <span className={`text-sm font-semibold ${c.pass ? 'text-emerald-800' : 'text-rose-700'}`}>
                    {c.label}
                  </span>
                </div>
              ))}
            ...
            {a.notes && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
                <p className="text-xs font-bold text-blue-700 uppercase tracking-wider mb-1">Screening Notes</p>
                <p className="text-sm text-blue-800">{a.notes}</p>
              </div>
            )}
          </div>
        ...
        <div className="px-6 py-5 border-t border-zinc-100 space-y-3 bg-white">
          <button
            onClick={() => onProceedToESign(a)}
            className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800
              text-white font-bold text-sm py-3.5 rounded-xl transition shadow-sm"
          >
            <PenTool className="h-4 w-4" />
...
interface FindTenantsPageProps {
  /** Called when owner clicks "Accept & Proceed to E-Sign" in the drawer */
  onNavigateToESign?: (applicant: Applicant) => void;
}
...
export const FindTenantsPage: React.FC<FindTenantsPageProps> = ({ onNavigateToESign }) => {
  ...
  // ── State controllers ────────────────────────────────────────────────────
  // activeFilter drives which metric card is highlighted AND pre-filters the table
  const [activeFilter, setActiveFilter]         = useState<ActiveFilter>('all');
  const [searchQuery,  setSearchQuery]          = useState('');
  // selectedApplicant: null = drawer closed; Applicant = drawer open with that record
  const [selectedApplicant, setSelectedApplicant] = useState<Applicant | null>(null);
  // Local status overrides — when owner declines, status updates client-side
  const [statusOverrides, setStatusOverrides]   = useState<Record<string, ScreeningStatus>>({});

  // Merge status overrides into the base data
  const allApplicants = useMemo(
    () => APPLICANTS.map(a => ({ ...a, status: statusOverrides[a.id] ?? a.status })),
    [statusOverrides]
  );
  ...
  const counts = useMemo(() => ({
    listings:     4,  // static — would come from GET /api/properties?status=live
    applications: allApplicants.length,
    screenings:   allApplicants.filter(a => a.status === 'Under Review' || a.status === 'Interview Scheduled').length,
  }), [allApplicants]);
  ...
  const filtered = useMemo(() => {
    ...
    // Filter by metric card tab
    if (activeFilter === 'applications') list = list.filter(a => a.status !== 'Screening Passed');
    if (activeFilter === 'screenings')   list = list.filter(a => a.status === 'Under Review' || a.status === 'Interview Scheduled');

    // Filter by search query against name or property title
    const q = searchQuery.trim().toLowerCase();
    if (q) list = list.filter(a =>
  ...
  /** Decline: update the local status state for that applicant row */
  const handleDecline = (id: string) => {
    setStatusOverrides(prev => ({ ...prev, [id]: 'Action Required' }));
  };

  /** Accept: open e-sign flow or call parent navigation callback */
  const handleProceedToESign = (applicant: Applicant) => {
  ...
  return (
    <div className="p-6 sm:p-8 bg-zinc-50 min-h-screen font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900">Find Tenants & Tenant Screening</h1>
          <p className="text-zinc-500 mt-1 text-sm">Manage rental applications and screen potential tenants.</p>
        </div>
        <button className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-semibold text-sm transition shadow-sm">
          <Plus className="h-4 w-4" /> Post New Listing
        </button>
      ...
      {/* ── Metric cards — clicking a card filters the table ── */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
        {METRIC_CARDS.map(card => {
          const Icon    = card.icon;
          const isActive = activeFilter === card.id;
          return (
            <button
              ...
              onClick={() =>
                ...
                setActiveFilter(prev => prev === card.id ? 'all' : card.id as ActiveFilter)
              }
              className={`flex items-center gap-4 p-6 rounded-xl border text-left transition-all
                ${isActive
                  ? 'bg-indigo-600 border-indigo-600 shadow-lg shadow-indigo-200'
                  : 'bg-white border-zinc-200 shadow-sm hover:border-indigo-300 hover:shadow-md'
                }`}
            >
              <div className={`p-3 rounded-xl shrink-0 ${isActive ? 'bg-white/20' : card.color}`}>
                <Icon className={`h-6 w-6 ${isActive ? 'text-white' : ''}`} />
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-zinc-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 border border-zinc-200 rounded-xl text-sm w-full sm:w-72
                focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/10 transition"
            />
            {searchQuery && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[640px]">
            <thead className="bg-zinc-50 border-b border-zinc-100">
              <tr>
                {['Applicant', 'Property', 'Applied', 'Screening', 'Status', ''].map(h => (
                  <th key={h} className="px-6 py-3.5 text-xs font-bold text-zinc-500 uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100">
              {filtered.length === 0 ? (
                ...
                // Clicking a row opens the detail drawer for that applicant
                <tr
                  key={a.id}
                ...
                >
                  ...
                  {/* Screening summary (credit score mini-bar) */}
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${a.creditScore >= 700 ? 'bg-emerald-500' : a.creditScore >= 650 ? 'bg-amber-400' : 'bg-rose-500'}`}
                          style={{ width: `${(a.creditScore / 850) * 100}%` }}
                        />
                      </div>
                      <span className="text-xs font-bold text-zinc-600">{a.creditScore}</span>
        ...
        <div className="px-6 py-3 border-t border-zinc-100 flex items-center justify-between">
          <p className="text-xs text-zinc-400">
            Click any row to view full applicant details and screening scores.
          </p>
          <div className="flex items-center gap-1.5 text-xs text-zinc-400">
            <TrendingUp className="h-3.5 w-3.5 text-emerald-500" />
            <span>{allApplicants.filter(a => a.status === 'Screening Passed').length} passed screening this week</span>
          </div>
        </div>
      </div>

      {/* ── Detail drawer (rendered when an applicant is selected) ── */}
      {selectedApplicant && (
        <ApplicantDrawer
          applicant={selectedApplicant}
          onClose={() => setSelectedApplicant(null)}
          onProceedToESign={handleProceedToESign}
          onDecline={handleDecline}
        />
      )}
    </div>
...
// Re-export the Applicant type so the parent dashboard can use it for navigation
export type { Applicant };
