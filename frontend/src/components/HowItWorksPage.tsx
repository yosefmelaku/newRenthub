    ...
    <div className="max-w-6xl mx-auto px-6 py-16 bg-slate-50 min-h-screen">
      <div className="text-center mb-16">
        ...
        <div className="inline-flex bg-white rounded-full p-1 border shadow-sm">
            <button onClick={() => setActiveTab('owner')} className={`px-6 py-2 rounded-full font-semibold ${activeTab === 'owner' ? 'bg-emerald-600 text-white' : 'text-slate-600'}`}>For Property Owners</button>
            <button onClick={() => setActiveTab('tenant')} className={`px-6 py-2 rounded-full font-semibold ${activeTab === 'tenant' ? 'bg-emerald-600 text-white' : 'text-slate-600'}`}>For Tenants</button>
        </div>
      </div>
